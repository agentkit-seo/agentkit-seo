...
- `stable`: The bio is short and should act as a compact landing page for niche, value, proof, and next step.
- `stable`: Profile image, header, bio, link path, pinned post, and recent topics should not contradict the user's claimed niche.
- `stable`: Alt Text is for accessibility first and AI interpretation second.
- `stable`: Threads should be used when the topic needs depth, not as a default format for every idea.
...
- `likely`: Staying online long enough to reply while discussion is active supports conversation quality.
...
- `disputed`: Exact live ranking weights, posting-frequency thresholds, and Phoenix or Grok-era production behavior should not be claimed as complete current truth.
...
- `likely`: Historical open-source recommendation material supports directional priors around conversation, dwell, author replies, candidate sourcing, ranking, diversity, and filtering.
